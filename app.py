    ...
    if category == "Basic Strategies":
        if strategy_type == "Long Call":
            ...
            selected_strike_index = st.select_slider(
                "Strike Selection",
                options=range(len(available_strikes)),
                value=max(0, atm_index-1),  # Default to slightly OTM put
                format_func=lambda i: f"${available_strikes[i]:.2f} ({'ITM' if available_strikes[i] > current_price else 'OTM' if available_strikes[i] < current_price else 'ATM'})"
            )
            selected_strike = available_strikes[selected_strike_index]
            ...
            # Allow user to specify a different entry price
            use_custom_price = st.checkbox("I sold this put at a different price")
            if use_custom_price:
                entry_premium = st.number_input(
                    "Your sale price", 
                    min_value=0.01, 
            ...
            with cols[1]:
                contract_value = entry_premium * 100
                st.metric("Contract Income", f"${contract_value:.2f}", delta=None)
            with cols[2]:
                breakeven = selected_strike - entry_premium
                st.metric("Breakeven", f"${breakeven:.2f}", delta=f"{((breakeven/current_price)-1)*100:.1f}%")
            ...
            selected_strike_index = st.select_slider(
                "Strike Selection",
                options=range(len(available_strikes)),
                value=min(len(available_strikes)-1, atm_index+1),  # Default to slightly OTM call
                format_func=lambda i: f"${available_strikes[i]:.2f} ({'ITM' if available_strikes[i] < current_price else 'OTM' if available_strikes[i] > current_price else 'ATM'})"
            )
            selected_strike = available_strikes[selected_strike_index]
            ...
            # Allow user to specify a different entry price
            use_custom_price = st.checkbox("I sold this call at a different price")
            if use_custom_price:
                entry_premium = st.number_input(
                    "Your sale price", 
                    min_value=0.01, 
            ...
            st.warning("⚠️ Naked calls have unlimited risk as the stock price can rise indefinitely!")
            ...
            # Display risk warning
            st.warning("⚠️ Naked puts have substantial risk if the stock price falls significantly!")

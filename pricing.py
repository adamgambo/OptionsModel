...
def calculate_portfolio_greeks(options_list, current_price, days_to_expiry, risk_free_rate=0.03):
    """
    Calculate the aggregate Greeks for a portfolio of options.
    ...
    # Calculate Greeks for each option and aggregate
    for option in options_list:
        ...
        position = option.get('position', 'long')
        strike = option.get('strike', current_price)
        iv = option.get('iv', 0.3)
        quantity = option.get('quantity', 1)
        
        # Calculate option Greeks
        greeks = calculate_greeks(option_type, current_price, strike, years, risk_free_rate, iv)
        
        # Adjust sign based on position (long/short)
        sign = 1 if position == 'long' else -1
        
        # Add to total Greeks, adjusted for quantity
        for greek in total_greeks:
            total_greeks[greek] += sign * greeks[greek] * quantity
    
    return total_greeks

# Additional utility functions
def calculate_iv_surface(options_chain, current_price):
    """
    Calculate the implied volatility surface across strikes and expirations.
    
    Parameters:
        options_chain (dict): Dictionary of option chains by expiration
        current_price (float): Current stock price
    
    Returns:
        DataFrame: DataFrame with IV surface data
    """
    surface_data = []
    
    for expiry, chain in options_chain.items():
        ...
        if not calls.empty:
            for _, row in calls.iterrows():
                moneyness = row['strike'] / current_price - 1
                surface_data.append({
                    'expiry': expiry,
                    'days': days_to_expiry,
